...
public class State extends GameState<Properties> {
	...
	public static class Builder {
		...
		public static Builder randomStateBuilder() {
			Properties gameProperties = new Properties(1+ (int)(Math.random()*4), 3 + (int) (Math.random()*3));
			Builder builder = new Builder(gameProperties,GameUtil.getCity("Atlanta")).gameStep(Pandemic.GameStep.play);
			...
			int researchCenterCount = builder.researchCenterCount+ (int) (Math.random()*(1+gameProperties.maxResearchCenterCounter-builder.researchCenterCount));
			...
			for(Desease desease: gameProperties.deseaseList) {
				...
				while(cubeInPlay>0) {
					City city = Reserve.getInstance().deseaseCityCardMap.get(desease).get((int) (Math.random()*(Reserve.getInstance().deseaseCityCardMap.get(desease).size())));
			...
			for(int i=0; i<builder.characterPositionMap.length; i++) {
				Desease desease = gameProperties.deseaseList.get((int) (Math.random()*(gameProperties.deseaseList.size())));
				City position = Reserve.getInstance().deseaseCityCardMap.get(desease).get((int) (Math.random()*(Reserve.getInstance().deseaseCityCardMap.get(desease).size())));
				builder.characterPositionMap[i]=position;
			}
			builder.currentHand(builder.characterHand[(int) (Math.random()*builder.characterHand.length)]);
			...
			for(int i=0; i<builder.characterHand.length; i++) {
				...
				while(k<handSize) {
					...
					if(card!=null)builder.characterHand[i].add(card);
					k++;
				}
			}
			builder.split();
			...
			while(epidemicCounter<epidemics) {
				...
				if(card instanceof EpidemicCard) epidemicCounter++;
			}
			int discardedCards = epidemics == 0 ? 0 : (int) (Math.random()*(builder.playerDeck.getSubdeckSize(epidemics-1)+1));
			for(int i=0; i<discardedCards; i++) {
				PlayerCard card = builder.playerDeck.draw();
				builder.playerDeck.discard(card);
				if(card instanceof EpidemicCard) epidemicCounter++;
			}
			int propagationDiscarded = (int) (Math.random()*(builder.propagationDeck.size()+1) * (gameProperties.propagationSpeed[builder.playerDeck.getEpidemicCounter()] - gameProperties.propagationSpeed[0])/gameProperties.propagationSpeed[gameProperties.propagationSpeed.length-1]);
		...
		public static Builder startStateBuilder(Properties gameProperties) {
			return new Builder(gameProperties, GameUtil.getCity("Atlanta")).deal().split().infect().gameStep(Pandemic.GameStep.play);
		}
		
		public static Builder blankStateBuilder(Properties gameProperties) {
			return new Builder(gameProperties, GameUtil.getCity("Atlanta")).split().gameStep(Pandemic.GameStep.play);
		}
	...
	public City getCharacterPosition(PlayedCharacter character) {
		return character.id < this.characterPositionMap.length ? this.characterPositionMap[character.id] : null;
	}

	public void nextPlayer() {
		currentHand = characterHand[(currentHand.getCharacter().id + 1) % gameProperties.numberOfPlayers];
		currentActionCount = gameProperties.maxActionCount;
	}
