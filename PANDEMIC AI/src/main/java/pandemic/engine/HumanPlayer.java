...
public class HumanPlayer extends AbstractPlayer {
	...
	private void action(Pandemic pandemic) {
		...
		logger.info("You are in "+pandemic.gameState.getCharacterPosition(playedCharacter).getName()+". There are "+ pandemic.gameState.getCityCubeCount(pandemic.gameState.getCharacterPosition(playedCharacter), pandemic.gameState.getCharacterPosition(playedCharacter).getDesease())+" cubes.");
		if(pandemic.gameState.hasResearchCenter(pandemic.gameState.getCharacterPosition(playedCharacter)) ) {
			logger.info("You are in a Research Center ! Other research centers are located in: "+pandemic.gameState.getResearchCenters().toString()+".");
		}
		...
		logger.info("Your only possible action are : drive-cityName,directFlight-cityName,charterFlight-cityName,shuttleFlight-cityName, treat, shareKnowledge-playerName, build, cure-deseaseName-card1,card2,card3,card4,card5");
		...
		if(str.startsWith("drive")) {
			...
			City city = GameUtil.getCity(cityName);
			if(city!=null) {
				gameAction = pandemic.gameState.getCharacterPosition(playedCharacter).getCharterFlight(city);
			} else {
				invalid=true;
			}
			...
			City city = GameUtil.getCity(cityName);
			if(city!=null) {
				gameAction = pandemic.gameState.getCharacterPosition(playedCharacter).getShuttleFlight(city);
			} else {
				invalid=true;
			}
		} else if (str.startsWith("treat")) {
			gameAction = pandemic.gameState.getCharacterPosition(playedCharacter).getTreat(pandemic.gameState.getCityCubeCount(pandemic.gameState.getCurrentPlayerPosition(), pandemic.gameState.getCurrentPlayerPosition().getDesease()), pandemic.gameState.isCured(pandemic.gameState.getCurrentPlayerPosition().getDesease()));
			...
			if(otherPlayer!=null) {
				gameAction = pandemic.gameState.getCharacterPosition(playedCharacter).getShareKnowledge(otherPlayer);
			} else {
				invalid=true;
			}
