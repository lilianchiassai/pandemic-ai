...
public class GameUtil {
  ...
  public static Predicate<? super City> getCityNamePredicate(String name) {
    return (city) -> (city.getClass().isAssignableFrom(City.class)
        && ((City) city).getName().equals(name));
  }

  public static Predicate<? super PlayedCharacter> getCharacterNamePredicate(String name) {
    return (playedCharacter) -> (playedCharacter.getClass().isAssignableFrom(PlayedCharacter.class)
        && ((PlayedCharacter) playedCharacter).getName().equals(name));
  }

  public static Predicate<? super Desease> getDeseaseNamePredicate(String name) {
    return (desease) -> (desease.getClass().isAssignableFrom(Desease.class)
        && ((Desease) desease).getName().equals(name));
  }

  /*
   * public static Graph<City, DefaultEdge> makeBidirectionnal(Graph<City, DefaultEdge> graph) { for
   * (City source: graph.vertexSet()) { Set<DefaultEdge> edges = graph.outgoingEdgesOf(source); for
   * (DefaultEdge edge : edges) { City target = graph.getEdgeTarget(edge); boolean comingBack =
   * graph.containsEdge(target, source); if(!graph.containsEdge(target, source)) {
   * graph.addEdge(target, source); } } } return graph; }
   */
  ...
  public static City getCity(String cityName) {
    Set<City> citySet = (Set<City>) Reserve.getInstance().getMap().values().stream()
        .filter(GameUtil.getCityNamePredicate(cityName)).collect(Collectors.toSet());
    if (citySet != null) {
  ...
  public static PlayedCharacter getPlayer(String playerName) {
    Set<PlayedCharacter> playerSet =
        (Set<PlayedCharacter>) Reserve.getInstance().getCharacterReserve().stream()
            .filter(GameUtil.getCharacterNamePredicate(playerName)).collect(Collectors.toSet());
    if (playerSet != null) {
  ...
  public static Desease getDesease(String deseaseName) {
    Set<Desease> deseaseSubSet = (Set<Desease>) Reserve.getInstance().getDeseaseSet().stream()
        .filter(GameUtil.getDeseaseNamePredicate(deseaseName)).collect(Collectors.toSet());
  ...
  public static int getActionWeight(State state, GameAction gameAction) {
    if (gameAction.getClass().isAssignableFrom(Drive.class)) {
      return (state.getCityCubeCount(((Drive) gameAction).getDestination(),
          ((Drive) gameAction).getDestination().getDesease())) * 10 + 10;
    } else if (gameAction.getClass().isAssignableFrom(DirectFlight.class)) {
      return state.getCityCubeCount(((DirectFlight) gameAction).getDestination(),
          ((DirectFlight) gameAction).getDestination().getDesease()) * 5 + 1;
    } else if (gameAction.getClass().isAssignableFrom(CharterFlight.class)) {
      return state.getCityCubeCount(((CharterFlight) gameAction).getDestination(),
          ((CharterFlight) gameAction).getDestination().getDesease()) * 5 + 1;
    } else if (gameAction.getClass().isAssignableFrom(ShuttleFlight.class)) {
      return (state.getCityCubeCount(((ShuttleFlight) gameAction).getDestination(),
          ((ShuttleFlight) gameAction).getDestination().getDesease())) * 10 + 5;
    } else if (gameAction.getClass().isAssignableFrom(Treat.class)) {
      return state.getCityCubeCount(state.getCurrentPlayerPosition(),
          state.getCurrentPlayerPosition().getDesease()) * 200;
      ...
      if (state.isCured(state.getCurrentPlayerPosition().getDesease())) {
        ...
        int current = state.getCurrentHand().stream()
            .filter((Predicate<? super Card>) GameUtil
                .getCityCardPredicate(state.getCurrentPlayerPosition().getDesease()))
            .collect(Collectors.toSet()).size();
        int other = state.getCharacterHand(((ShareKnowledge) gameAction).getCharacter()).stream()
            .filter((Predicate<? super Card>) GameUtil
                .getCityCardPredicate(state.getCurrentPlayerPosition().getDesease()))
            .collect(Collectors.toSet()).size();
      ...
      if (((Discard) gameAction).getCard() instanceof CityCard) {
        if (state.isCured(((CityCard) ((Discard) gameAction).getCard()).getCity().getDesease())) {
          ...
          for (Hand hand : state.getAllCharacterHand()) {
            int current = hand.stream()
                .filter((Predicate<? super Card>) GameUtil.getCityCardPredicate(
                    ((CityCard) ((Discard) gameAction).getCard()).getCity().getDesease()))
                .collect(Collectors.toSet()).size();
            max = current > max ? current : max;
            bestHand = bestHand == null || current > max ? hand : bestHand;
          }
  ...
  public static PlayerCard getCard(int cityId) {
    Set<PlayerCard> result = Reserve.getInstance().getPlayerCardReserve().stream()
        .filter(card -> card instanceof CityCard && ((CityCard) card).getCity().id == cityId)
