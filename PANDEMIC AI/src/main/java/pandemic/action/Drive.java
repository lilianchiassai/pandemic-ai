...
public class Drive extends MoveAction {
	...
	public boolean perform(Pandemic pandemic) {
		if(super.perform(pandemic)) {
			GameUtil.log(pandemic, GameAction.logger, pandemic.gameState.getCurrentPlayer().getName()+" drives from "+origin.getName()+" to "+ destination.getName()+" in "+this.actionCost+" hours");
