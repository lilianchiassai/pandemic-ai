...
public class CharterFlight extends MoveAction {
	...
	public boolean canPerform(Pandemic pandemic) {
		if(pandemic.gameState.getCurrentActionCount() >= this.actionCost) {
			if(pandemic.gameState.getCurrentPlayerPosition()==origin && pandemic.gameState.getCurrentHand().contains(origin.getCityCard())) {
				return true;
			}
		}
		return false;
	...
	public boolean perform(Pandemic pandemic) {
		CityCard cityCard = pandemic.gameState.getCurrentHand().getCityCard(pandemic.gameState.getCurrentPlayerPosition());
		if(cityCard != null && super.perform(pandemic)) {
			pandemic.gameState.getCurrentHand().removeAndDiscard(pandemic.gameState, cityCard);
			GameUtil.log(pandemic, GameAction.logger, pandemic.gameState.getCurrentPlayer().getName()+" takes a charter flight from "+origin.getName()+" to "+ destination.getName());			
