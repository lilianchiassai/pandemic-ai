...
public class DirectFlight extends MoveAction {
	...
	public boolean canPerform(Pandemic pandemic) {
		if(pandemic.gameState.getCurrentActionCount() >= this.actionCost) {
			if(pandemic.gameState.getCurrentPlayerPosition()==origin && pandemic.gameState.getCurrentHand().contains(destination.getCityCard())) {
				return true;
			}
		}
		return false;
	...
	public boolean perform(Pandemic pandemic) {
		...
		if(cityCard != null && super.perform(pandemic)) {
			...
			GameUtil.log(pandemic, GameAction.logger, pandemic.gameState.getCurrentPlayer().getName()+" flies directly from "+origin.getName()+" to "+ destination.getName());
	...
	public void cancel(Pandemic pandemic) {
		...
		pandemic.gameState.getPlayerDeck().getDiscardPile().remove(destination.getCityCard());
	}
