...
public class Cure extends StaticAction {
	...
	public boolean canPerform(Pandemic pandemic) {
		if(pandemic.gameState.getCurrentActionCount() >= this.actionCost) {
			if(pandemic.gameState.getCurrentHand().size() > this.set.size() && !pandemic.gameState.isCured(desease) && pandemic.gameState.hasResearchCenter(pandemic.gameState.getCurrentPlayerPosition())) {
	...
	public boolean perform(Pandemic pandemic) {
		if(pandemic.gameState.getCurrentHand().size() > this.set.size() && !pandemic.gameState.isCured(desease) && pandemic.gameState.hasResearchCenter(origin)) {
			if(pandemic.gameState.getCurrentHand().containsAll(this.set)) {
				pandemic.gameState.getCurrentHand().removeAndDiscard(pandemic.gameState, this.set);
				GameUtil.log(pandemic, GameAction.logger, pandemic.gameState.getCurrentPlayer().getName()+" finds a Cure in "+origin.getName()+" for the "+desease.getName()+" desease.");
