...
public class GiveKnowledge extends ShareKnowledge {
    ...
    if (pandemic.gameState.getCurrentHand().getCharacter() != this.playedCharacter) {
      if (pandemic.gameState.getCurrentActionCount() >= this.actionCost) {
        ...
        if (pandemic.gameState.getCurrentPlayerPosition() == pandemic.gameState
            ...
            && pandemic.gameState.getCurrentHand().contains(cityCard)) {
          return true;
        }
      }
    }
  ...
  public boolean perform(Pandemic pandemic) {
    if (pandemic.gameState.getCurrentHand().getCharacter() != this.playedCharacter) {
      CityCard cityCard = pandemic.gameState.getCurrentPlayerPosition().getCityCard();
      if (cityCard != null
          && pandemic.gameState.getCurrentPlayerPosition() == pandemic.gameState
              .getCharacterPosition(playedCharacter)
          && pandemic.gameState.getCurrentHand().contains(cityCard)) {
        if (super.perform(pandemic)) {
          ...
          return pandemic.giveCard(pandemic.gameState.getCurrentPlayer(), playedCharacter,
              cityCard);
        }
      }
    }
