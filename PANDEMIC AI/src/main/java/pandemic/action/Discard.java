...
public class Discard extends GameAction {
	...
	public boolean perform(Pandemic pandemic) {
		if(canPerform(pandemic) && super.perform(pandemic)) {
			GameUtil.log(pandemic, GameAction.logger, pandemic.gameState.getCurrentPlayer().getName()+" discards "+ card.getTitle() +".");
			return pandemic.gameState.getCharacterHand(playedCharacter).removeAndDiscard(pandemic.gameState,(CityCard) card);
		}
		return false;
	}
