...
public class Treat extends StaticAction {
	...
	public boolean canPerform(Pandemic pandemic) {
		if(pandemic.gameState.getCurrentActionCount() >= this.actionCost) {
			if(healAll) {
				if(pandemic.gameState.getCityCubeCount(pandemic.gameState.getCurrentPlayerPosition(), desease)==strength && pandemic.gameState.isCured(desease)) {
					return true;
				}
				return false;
	...
	public boolean perform(Pandemic pandemic) {
		if(canPerform(pandemic) && pandemic.gameState.getCityCubeCount(origin, desease)>=strength) {
			GameUtil.log(pandemic, GameAction.logger, pandemic.gameState.getCurrentPlayer().getName()+" treats "+desease.getName()+" desease in "+origin.getName());
			if(healAll && pandemic.gameState.getCityCubeCount(origin, desease)==strength && pandemic.gameState.isCured(desease) && super.perform(pandemic)) {
				pandemic.gameState.removeCube(origin, desease, strength);
				pandemic.checkEradicated(desease);
				return true;
			} 
			if(!healAll && super.perform(pandemic)) {
				return pandemic.gameState.removeCube(pandemic.gameState.getCurrentPlayerPosition(), desease, strength);
			}
		}
		return false;
	}
