...
public class ReceiveKnowledge extends ShareKnowledge {
  ...
  public boolean canPerform(Pandemic pandemic) {
    if (pandemic.gameState.getCurrentHand().getCharacter() != this.playedCharacter) {
      if (pandemic.gameState.getCurrentActionCount() >= this.actionCost) {
        ...
        if (pandemic.gameState.getCurrentPlayerPosition() == pandemic.gameState
            .getCharacterPosition(playedCharacter)
            && pandemic.gameState.getCharacterHand(playedCharacter).contains(cityCard)) {
          return true;
        }
      }
    }
  ...
  public boolean perform(Pandemic pandemic) {
    if (pandemic.gameState.getCurrentHand().getCharacter() != this.playedCharacter) {
      ...
      if (cityCard != null && origin == pandemic.gameState.getCharacterPosition(playedCharacter)
          && pandemic.gameState.getCharacterHand(playedCharacter).contains(cityCard)) {
        if (super.perform(pandemic)) {
          GameUtil.log(pandemic, GameAction.logger, pandemic.gameState.getCurrentPlayer().getName()
              + " gives card " + cityCard.getTitle() + " to " + playedCharacter.getName());
          return pandemic.giveCard(playedCharacter, pandemic.gameState.getCurrentPlayer(),
              cityCard);
        }
      }
    }
