...
public class Build extends StaticAction {
	...
	public boolean canPerform(Pandemic pandemic) {
		if(pandemic.gameState.getCurrentActionCount() >= this.actionCost) {
			if(!pandemic.gameState.hasResearchCenter(origin) && pandemic.gameState.getResearchCenterCount()<pandemic.gameState.gameProperties.maxResearchCenterCounter && pandemic.gameState.getCurrentHand().contains(origin.getCityCard())) {
				return true;
			}
		}
		return false;
		...
		if(pandemic.gameState.getCurrentHand().contains(origin.getCityCard()) ) {
			if(canPerform(pandemic)) {
				if(pandemic.gameState.addResearchCenter(pandemic.gameState.getCurrentPlayerPosition())) {
					GameUtil.log(pandemic, GameAction.logger, pandemic.gameState.getCurrentPlayer().getName()+" builds a new Research Center in "+origin.getName()+".");
					pandemic.gameState.getCurrentHand().removeAndDiscard(pandemic.gameState, origin.getCityCard());
