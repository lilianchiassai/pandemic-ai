...
public class ShuttleFlight extends MoveAction {
  ...
  public boolean canPerform(Pandemic pandemic) {
    if (pandemic.gameState.getCurrentActionCount() >= this.actionCost) {
      if (pandemic.gameState.hasResearchCenter(origin)
          && pandemic.gameState.hasResearchCenter(destination)) {
        return true;
      }
    }
    return false;
  ...
  public boolean perform(Pandemic pandemic) {
    if (pandemic.gameState.hasResearchCenter(pandemic.gameState.getCurrentPlayerPosition())
        && pandemic.gameState.hasResearchCenter(destination) && super.perform(pandemic)) {
      GameUtil.log(pandemic, GameAction.logger, pandemic.gameState.getCurrentPlayer().getName()
          + " flies in a shuttle from " + origin.getName() + " to " + destination.getName());
